...
function openStoreCustomOrder(){
  ...
  $('#storeCustomOrderForm').onsubmit=async e=>{
    ...
    try{const name=$('#storeCustomName').value.trim(),whats=$('#storeCustomWhats').value.trim(),project=$('#storeCustomProject').value.trim(),note=$('#storeCustomNote').value.trim();rememberStoreContact(name,whats);const {data,error}=await supabaseClient.rpc('submit_store_custom_request',{p_store_token:storePublicTokenCache||storeTokenFromHash(),p_customer_name:name,p_whatsapp:whats,p_project:project,p_note:note});if(error)throw error;closeModal();toast(`Pedido #${data} recebido. O estúdio entrará em contato.`, 'success');}
    catch(err){toast(err?.message||'Não foi possível enviar o pedido.','error');btn.disabled=false;}
  };
}
async function openStoreChat(){
  const saved=getStoreContact();
  if(saved.name&&saved.whats){return launchStoreChat(saved.name,saved.whats);}
  modal(`<div class="modal-head"><div><span class="eyebrow">CONVERSA</span><h2>Fale com o designer</h2><p class="muted">Não precisa criar conta. Guardo apenas seu nome e WhatsApp neste dispositivo para facilitar a próxima conversa.</p></div><button class="close-modal" data-close-modal>×</button></div><form id="storeChatForm" class="store-public-form"><div class="two-col"><label>Seu nome<input id="storeChatName" required></label><label>WhatsApp<input id="storeChatWhats" inputmode="tel" required></label></div><div class="modal-actions"><button type="button" class="btn secondary" data-close-modal>Cancelar</button><button class="btn primary" type="submit">Continuar para conversa →</button></div></form>`);
  $('#storeChatForm').onsubmit=async e=>{e.preventDefault();const name=$('#storeChatName').value.trim(),whats=$('#storeChatWhats').value.trim();rememberStoreContact(name,whats);closeModal();launchStoreChat(name,whats);};
}
