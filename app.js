...
const KEYS = { accounts:`${APP}:accounts`, user:`${APP}:user`, theme:`${APP}:theme`, designer:`${APP}:designer`, orders:`${APP}:orders`, clients:`${APP}:clients`, quotes:`${APP}:quotes`, notifications:`${APP}:notifications`, trash:`${APP}:trash`, deletedRemote:`${APP}:deleted-remote`, deletedRemoteFingerprints:`${APP}:deleted-remote-fingerprints` };
...
function persist(){
  if(currentUser){ write(scopedKey('rafahstudio:orders'),orders); write(scopedKey('rafahstudio:clients'),clients); write(scopedKey('rafahstudio:quotes'),quotes); write(scopedKey('rafahstudio:notifications'),notifications); write(scopedKey('rafahstudio:designer'),designer); write(scopedKey('rafahstudio:trash'),trash); write(scopedKey('rafahstudio:deletedRemote'),deletedRemoteIds); write(scopedKey('rafahstudio:deletedRemoteFingerprints'),deletedRemoteFingerprints); }
...
function setupPublic(){let people=[];function paintPeople(){ $('#peopleList').innerHTML=people.map((p,i)=>`<div class="person-row"><div class="person-num">${i+1}</div><label>Nome<input data-person-name="${i}" value="${esc(p.name)}" required></label><label>Participação / informação<input data-person-info="${i}" value="${esc(p.info)}"></label><label class="person-photo">Foto<input data-person-photo="${i}" type="file" accept="image/*"><small>${p.photo?.name||'Opcional'}</small></label><button type="button" class="icon-action danger" data-remove-person="${i}">×</button></div>`).join('');}
 ...
 }; $('#closePublicBtn').onclick=()=>{ $('#publicSuccess').classList.add('hidden'); $('#publicFormView').classList.remove('hidden'); $('#briefingForm').reset(); people=[]; paintPeople(); $('#filePreview').innerHTML=''; $('#pubFiles')._files=[]; $('#publicMessage').textContent=''; const btn=$('#briefingForm button[type=submit]'); if(btn){btn.disabled=false;btn.textContent='Enviar briefing →';} };paintPeople(); }
