...
drop function if exists public.submit_order_message_with_file(text,text,text,jsonb);
create or replace function public.submit_order_message_with_file(p_tracking_token text,p_message text,p_image_url text,p_meta jsonb default '{}'::jsonb)
returns bigint language plpgsql security definer set search_path=public as $$
declare t public.order_tracking%rowtype; v_id bigint;
begin
  select * into t from public.order_tracking where tracking_token=p_tracking_token limit 1;
  if t.tracking_token is null then raise exception 'Pedido não encontrado.'; end if;
  if t.status in ('Pago','Finalizado') then raise exception 'Este pedido já foi finalizado.'; end if;
  if length(trim(coalesce(p_message,'')))<1 and length(trim(coalesce(p_image_url,'')))<1 then raise exception 'Digite uma mensagem ou selecione um arquivo.'; end if;
  insert into public.order_tracking_events(tracking_token,owner_secret,order_id,author,kind,message,image_url,meta)
  values(t.tracking_token,t.owner_secret,t.order_id,'client','message',trim(coalesce(p_message,'')),trim(coalesce(p_image_url,'')),coalesce(p_meta,'{}'::jsonb)) returning id into v_id;
  update public.order_tracking set updated_at=now() where tracking_token=p_tracking_token;
  return v_id;
end $$;
grant execute on function public.submit_order_message_with_file(text,text,text,jsonb) to anon,authenticated;

drop function if exists public.submit_order_alteration_with_file(text,text,text,jsonb);
create or replace function public.submit_order_alteration_with_file(p_tracking_token text,p_message text,p_image_url text,p_meta jsonb default '{}'::jsonb)
returns bigint language plpgsql security definer set search_path=public as $$
declare t public.order_tracking%rowtype; v_id bigint;
begin
  select * into t from public.order_tracking where tracking_token=p_tracking_token limit 1;
  ...
  if t.status in ('Pago','Finalizado') then raise exception 'Este pedido já foi finalizado.'; end if;
  insert into public.order_tracking_events(tracking_token,owner_secret,order_id,author,kind,message,image_url,meta)
  values(t.tracking_token,t.owner_secret,t.order_id,'client','alteration',trim(p_message),trim(coalesce(p_image_url,'')),coalesce(p_meta,'{}'::jsonb)) returning id into v_id;
  update public.order_tracking set status='Alteração',updated_at=now() where tracking_token=p_tracking_token;
  return v_id;
end $$;
grant execute on function public.submit_order_alteration_with_file(text,text,text,jsonb) to anon,authenticated;
...
-- ==========================================================
-- RAFAHSTUDIO 1.6 — PRODUTOS / SERVIÇOS DO DESIGNER
-- Cadastro privado por owner_secret, com preço e imagem de referência.
-- ==========================================================
...
create index if not exists product_items_owner_secret_idx
  on public.product_items(owner_secret);

alter table public.product_items enable row level security;

-- O frontend usa funções SECURITY DEFINER; a tabela permanece bloqueada diretamente.
drop function if exists public.create_product(text,text,text,text,numeric,text);
...
declare v_id bigint;
begin
  if nullif(trim(coalesce(p_owner_secret,'')),'') is null then
    raise exception 'Sessão inválida.';
  end if;
  if nullif(trim(coalesce(p_name,'')),'') is null then
    raise exception 'Informe o nome do produto.';
  end if;
  insert into public.product_items(owner_secret,name,category,description,price,image_url)
  values(
    p_owner_secret,
    trim(p_name),
    coalesce(nullif(trim(p_category),''),'Outro'),
...
grant execute on function public.create_product(text,text,text,text,numeric,text) to authenticated;

drop function if exists public.update_product(text,bigint,text,text,text,numeric,text);
...
begin
  ...
  set name=trim(p_name),
      category=coalesce(nullif(trim(p_category),''),'Outro'),
      description=coalesce(p_description,''),
      price=greatest(coalesce(p_price,0),0),
      image_url=coalesce(p_image_url,''),
      updated_at=now()
  where id=p_id
    and owner_secret=p_owner_secret;
  if not found then
    raise exception 'Produto não encontrado ou sem permissão.';
  end if;
end $$;
grant execute on function public.update_product(text,bigint,text,text,text,numeric,text) to authenticated;

drop function if exists public.delete_product(text,bigint);
create or replace function public.delete_product(p_owner_secret text,p_id bigint)
returns void
language plpgsql
security definer
set search_path=public
...
grant execute on function public.delete_product(text,bigint) to authenticated;

drop function if exists public.get_products_for_owner(text);
create or replace function public.get_products_for_owner(p_owner_secret text)
returns table(
  id bigint,
  name text,
  category text,
...
set search_path=public
as $$
  select id,name,category,description,price,image_url,created_at,updated_at
...
-- e solicitações de compra/pedido.
-- ==========================================================
...
create index if not exists store_links_owner_idx on public.store_links(owner_secret);

create or replace function public.register_store_link(p_store_token text,p_owner_secret text)
returns void language plpgsql security definer set search_path=public as $$
declare v_owner text;
begin
  if length(coalesce(p_store_token,'')) < 20 or length(coalesce(p_owner_secret,'')) < 20 then raise exception 'Token inválido'; end if;
  select owner_secret into v_owner from public.store_links where store_token=p_store_token for update;
  if v_owner is not null and v_owner<>p_owner_secret then raise exception 'Este link já pertence a outro workspace'; end if;
  insert into public.store_links(store_token,owner_secret) values(p_store_token,p_owner_secret) on conflict(store_token) do nothing;
end $$;
grant execute on function public.register_store_link(text,text) to authenticated;

alter table public.product_items add column if not exists is_published boolean not null default false;
create index if not exists product_items_published_idx on public.product_items(owner_secret,is_published,created_at desc);

create or replace function public.set_product_published(p_owner_secret text,p_id bigint,p_is_published boolean)
returns void language plpgsql security definer set search_path=public as $$
begin
  update public.product_items set is_published=coalesce(p_is_published,false),updated_at=now() where id=p_id and owner_secret=p_owner_secret;
  if not found then raise exception 'Produto não encontrado ou sem permissão.'; end if;
end $$;
...
$$;
grant execute on function public.get_products_for_store(text) to anon,authenticated;
...
create table if not exists public.store_customers (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(owner_secret, whatsapp_norm)
);
create index if not exists store_customers_owner_idx on public.store_customers(owner_secret,updated_at desc);

create or replace function public.register_store_customer(p_store_token text,p_customer_name text,p_whatsapp text,p_email text)
returns bigint language plpgsql security definer set search_path=public as $$
declare v_owner text;v_id bigint;v_whats text;
begin
  if nullif(trim(coalesce(p_customer_name,'')),'') is null then raise exception 'Informe seu nome.'; end if;
  v_whats=regexp_replace(coalesce(p_whatsapp,''),'\D','','g');
  if length(v_whats)<8 then raise exception 'Informe um WhatsApp válido.'; end if;
  ...
  insert into public.store_customers(owner_secret,store_token,name,whatsapp,whatsapp_norm,email,updated_at)
  values(v_owner,p_store_token,trim(p_customer_name),trim(p_whatsapp),v_whats,coalesce(trim(p_email),''),now())
  on conflict(owner_secret,whatsapp_norm) do update set name=excluded.name,whatsapp=excluded.whatsapp,email=excluded.email,store_token=excluded.store_token,updated_at=now()
  returning id into v_id;
  return v_id;
end $$;
grant execute on function public.register_store_customer(text,text,text,text) to anon,authenticated;
...
alter table public.store_order_requests add column if not exists customer_id bigint references public.store_customers(id);
create index if not exists store_order_requests_owner_idx on public.store_order_requests(owner_secret,created_at desc);
...
returns bigint language plpgsql security definer set search_path=public as $$
declare v_owner text;v_id bigint;v_total numeric(12,2);v_valid int;v_customer bigint;
begin
  if nullif(trim(coalesce(p_customer_name,'')),'') is null then raise exception 'Informe seu nome.'; end if;
  select owner_secret into v_owner from public.store_links where store_token=p_store_token limit 1;
  if v_owner is null then raise exception 'Vitrine não encontrada.'; end if;
  v_customer=public.register_store_customer(p_store_token,p_customer_name,p_whatsapp,p_email);
  select count(*) into v_valid from jsonb_array_elements(coalesce(p_items,'[]'::jsonb)) j join public.product_items p on p.id=(j->>'id')::bigint and p.owner_secret=v_owner and p.is_published=true;
  if v_valid=0 then raise exception 'Os produtos selecionados não estão mais disponíveis.'; end if;
  select coalesce(sum(p.price*greatest(coalesce((j->>'qty')::int,1),1)),0) into v_total from jsonb_array_elements(coalesce(p_items,'[]'::jsonb)) j join public.product_items p on p.id=(j->>'id')::bigint and p.owner_secret=v_owner and p.is_published=true;
  insert into public.store_order_requests(owner_secret,store_token,customer_id,customer_name,whatsapp,email,items,total,note)
  values(v_owner,p_store_token,v_customer,trim(p_customer_name),coalesce(trim(p_whatsapp),''),coalesce(trim(p_email),''),coalesce(p_items,'[]'::jsonb),v_total,coalesce(p_note,'')) returning id into v_id;
  return v_id;
end $$;
...
returns bigint language plpgsql security definer set search_path=public as $$
declare v_owner text; v_id bigint; v_customer bigint;
begin
  if nullif(trim(coalesce(p_customer_name,'')),'') is null then raise exception 'Informe seu nome.'; end if;
  ...
  select owner_secret into v_owner from public.store_links where store_token=p_store_token limit 1;
  if v_owner is null then raise exception 'Vitrine não encontrada.'; end if;
  v_customer=public.register_store_customer(p_store_token,p_customer_name,p_whatsapp,'');
  insert into public.store_order_requests(owner_secret,store_token,customer_id,customer_name,whatsapp,email,items,total,note)
  values(v_owner,p_store_token,v_customer,trim(p_customer_name),trim(p_whatsapp),'',jsonb_build_array(jsonb_build_object('id',null,'qty',1,'name','Pedido personalizado','price',0)),0,concat('Projeto: ',trim(p_project),case when nullif(trim(coalesce(p_note,'')),'') is not null then E'\n\nDetalhes: '||trim(p_note) else '' end)) returning id into v_id;
  return v_id;
end $$;
...
language sql security definer set search_path=public as $$
  select id,customer_name,whatsapp,email,items,total,note,status,created_at
...
create or replace function public.get_public_token_for_store(p_store_token text)
returns text language sql security definer set search_path=public as $$
  select l.public_token
  from public.store_links s
  join public.briefing_links l on l.owner_secret=s.owner_secret
  where s.store_token=p_store_token
  limit 1;
$$;
grant execute on function public.get_public_token_for_store(text) to anon,authenticated;
